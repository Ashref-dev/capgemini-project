...
import { AiChat02Icon, Add01Icon, Menu01Icon, Link01Icon, Tick02Icon } from "@hugeicons/core-free-icons"
...
export function AgentHeader({
  ...
  const copyLink = React.useCallback(() => {
    ...
    void navigator.clipboard.writeText(url).then(
      () => {
        setCopied(true)
        toast.success("Lien copié", { description: "Le lien de la conversation est dans le presse-papiers." })
        window.setTimeout(() => setCopied(false), 1600)
      },
      () => toast.error("Copie impossible", { description: "Impossible d'accéder au presse-papiers." }),
    )
  }, [activeThreadId])

  const title = threadTitle?.trim() || (activeThreadId ? "Conversation" : "Nouvelle conversation")
    ...
    <header className="flex h-14 items-center justify-between gap-2 border-b border-border bg-background px-3 sm:px-4">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        {activeThreadId != null ? (
          ...
          >
            <HugeiconsIcon icon={copied ? Tick02Icon : Link01Icon} className="h-3.5 w-3.5" />
        ...
        <button
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-2.5 text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
        >
