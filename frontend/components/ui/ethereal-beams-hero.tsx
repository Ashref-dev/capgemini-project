...
const Beams: FC<BeamsProps> = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB("#001A3A")),
  ...
  return (
    <CanvasWrapper>
      ...
      <color attach="background" args={["#001A3A"]} />
...
// ============================================================================
// HERO BUTTON COMPONENT (local, with shimmer effect)
// ============================================================================
...
const HeroButton = ({ variant = "default", size = "sm", className = "", children, ...props }: HeroButtonProps) => {
  ...
  const variants = {
    default: "bg-[#0070AD] text-white hover:bg-[#005a8e]",
...
export function EtherealBeamsHero() {
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#001A3A]">
      {/* Beams Background */}
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Link href="/auth/sign-in">
                <HeroButton size="sm" className="shadow-lg shadow-[#0070AD]/30 font-semibold">
                  Se connecter
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mb-8 inline-flex items-center rounded-full bg-white/5 backdrop-blur-xl border border-white/10 px-4 py-2 text-sm text-white/90">
              <Star className="mr-2 h-4 w-4 text-[#12ABDB]" />
              Together, we build more ·{" "}
              <Link href="/solutions" className="ml-1 underline underline-offset-2 hover:text-white transition-colors">
                Read more →
              </Link>
            </div>
            ...
            <h1 className="mb-6 text-4xl font-bold tracking-tight text-white sm:text-6xl lg:text-7xl">
              ...
              <span className="block bg-gradient-to-r from-[#0070AD] to-[#12ABDB] bg-clip-text text-transparent pb-2">
                into opportunities
              </span>
            </h1>

            {/* Subtitle */}
            <p className="mb-10 text-lg leading-8 text-white/80 sm:text-xl lg:text-2xl max-w-3xl mx-auto font-light">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
              <Link href="/success-stories/apply">
                <HeroButton size="lg" className="shadow-2xl shadow-[#0070AD]/30 font-semibold">
                  Get Started Now
                  <ArrowRight className="ml-2 h-5 w-5" />
                </HeroButton>
              </Link>
              <Link href="/solutions">
                <HeroButton variant="outline" size="lg" className="font-semibold">
                  Solutions
                </HeroButton>
              </Link>
            </div>

            {/* Stats Cards — inspirées de WhyCapgemini */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
              {[
                { value: "60 ans", label: "d'expertise mondiale en transformation digitale" },
                { value: "340 000+", label: "collaborateurs dans le monde" },
                { value: "85%", label: "des 200 plus grandes entreprises nous font confiance" },
                { value: "13x", label: "nommé parmi les entreprises les plus éthiques" },
              ].map((stat, i) => (
                <div
                  key={i}
                  className="rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 px-4 py-5 text-center hover:bg-white/10 hover:border-white/20 transition-all"
                >
      ...
      <div className="absolute inset-0 z-0 bg-gradient-to-t from-[#001A3A]/60 via-transparent to-[#001A3A]/30 pointer-events-none" />
    </div>
  )
}
